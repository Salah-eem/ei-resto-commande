...
const MenuManager: React.FC = () => {
  ...
  const [prodData, setProdData] = useState<
    Partial<Product & { file: File | null }>
  >({
    productType: ProductType.SINGLE_PRICE,
    ingredients: [],
    file: null,
  });
  ...
  const categorySchema = Yup.object().shape({
    ...
    idx: Yup.number()
      ...
      .test("unique-idx", "Index must be unique", async function (value) {
  ...
  const productSchema = Yup.object().shape({
    ...
    basePrice: Yup.number().when("productType", {
      is: ProductType.SINGLE_PRICE,
      then: (s) =>
        s.required("Required").moreThan(0, "Price must be greater than 0"),
  ...
  const validateIngredientName = async (
    name: string,
    ingredientId?: string
  ) => {
    try {
  ...
  const ingredientSchema = Yup.object({
    name: Yup.string()
      ...
      .test("unique", "Ingredient name must be unique", async function (value) {
        ...
        // Délai pour éviter trop de requêtes
        await new Promise((resolve) => setTimeout(resolve, 300));
        const isUnique = await validateIngredientName(
          value,
          ingredientEdit?._id
        );
        return isUnique;
    ...
    try {
      ...
      if (catEdit) {
        actionResult = await dispatch(
          updateCategory({
            id: catEdit._id,
            data: { name: catName, idx: idxValue },
          }) as any
        );
      } else {
        actionResult = await dispatch(
          addCategory({ name: catName, idx: idxValue }) as any
        );
      }
      if (!actionResult.error) {
        // dispatch(fetchCategories() as any);
        showSnackbar(
          catEdit ? "Category updated!" : "Category added!",
          "success"
        );
      }
      ...
      showSnackbar(
        e?.response?.data?.message || "Error saving category",
        "error"
      );
    }
  };
  const handleCatDelete = async (cat: Category) => {
    ...
    try {
      ...
      showSnackbar(
        e?.response?.data?.message || "Error deleting category",
        "error"
      );
    }
  };
  ...
  const handleProdSave = async (
    ...
    try {
      let payload: any = {
        ...values,
        category:
          typeof values.category === "object" && values.category?._id
            ? values.category._id
            : values.category,
        ingredients: Array.isArray(values.ingredients)
          ? values.ingredients.map((ing: Ingredient) =>
              typeof ing === "string" ? ing : ing._id || ing
            )
          : [],
      };
      ...
      if (prodEdit) {
        actionResult = await dispatch(
          updateProduct({ id: prodEdit._id, data: payload }) as any
        );
      } else {
      ...
      if (!actionResult.error) {
        // Upload image after saving product if a file was selected
        if (values.file && actionResult.payload && actionResult.payload._id) {
          const formData = new FormData();
          formData.append("file", values.file);
          try {
            await dispatch(
              updateProductImage({
                id: actionResult.payload._id,
                formData,
              }) as any
            );
            showSnackbar(
          ...
          showSnackbar(
            prodEdit ? "Product updated!" : "Product added!",
            "success"
          );
        }
      }
      ...
      setErrors &&
        setErrors({
          name: e?.response?.data?.message || "Error saving product",
        });
  ...
  const handleCategoryDragEnd = async (result: DropResult) => {
    ...
    try {
      // await api.post("/category/reorder", {
      //   updates: updated.map(({ _id, idx }) => ({ _id, idx })),
      // });
      await dispatch(
        updateCategoryOrder(updated.map(({ _id, idx }) => ({ _id, idx })))
      );
      ...
      showSnackbar(
        e?.response?.data?.message || "Error updating category order",
        "error"
      );
    }
  };
  ...
  const handleIngredientSubmit = async (
    ...
    try {
      ...
      if (ingredientEdit) {
        actionResult = await dispatch(
          updateIngredient({ id: ingredientEdit._id, data: ingredient }) as any
        );
      } else {
        actionResult = await dispatch(addIngredient(ingredient) as any);
      }

      // Upload image after saving ingredient if a file was selected
      if (values.file && actionResult.payload && actionResult.payload._id) {
        const formData = new FormData();
        formData.append("file", values.file);
        try {
          ...
          showSnackbar(
            ingredientEdit
              ? "Ingredient updated with image!"
              : "Ingredient added with image!",
            "success"
          );
        } catch (err) {
          showSnackbar(
            ingredientEdit
              ? "Ingredient updated but image upload failed"
              : "Ingredient added but image upload failed",
            "warning"
          );
        }
      } else {
        showSnackbar(
          ingredientEdit ? "Ingredient updated!" : "Ingredient added!",
          "success"
        );
      }
      ...
      setErrors({
        name: e?.response?.data?.message || "Error saving ingredient",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  const handleIngredientDelete = async (ingredient: Ingredient) => {
    ...
    try {
      const actionResult = await dispatch(
        deleteIngredient(ingredient._id) as any
      );
      if (!actionResult.error) {
        // dispatch(fetchIngredients() as any);
      ...
      showSnackbar(
        e?.response?.data?.message || "Error deleting ingredient",
        "error"
      );
    }
  };
  ...
  return (
    <ProtectRoute allowedRoles={[Role.Admin]}>
      ...
      >
        ...
        <Paper sx={{ p: 3, borderRadius: 3 }}>
              ...
              >
                <Button
                  ...
                  onClick={() => {
                    const maxIdx =
                      categories.length > 0
                        ? Math.max(...categories.map((c) => c.idx))
                        : 0;
                    setCatEdit(null);
                    setCatName("");
                    setCatIdx(maxIdx + 1);
                      ...
                      {prods.map((prod) => (
                        ...
                        >
                          <Box>
                            ...
                            {prod.productType === ProductType.MULTIPLE_SIZES &&
                              prod.sizes && (
                                <Box>
                            ...
                            <Typography
                              ...
                              color="text.secondary"
                            >
                              {capitalizeFirstLetter(prod.category?.name)}
                            </Typography>
                          </Box>
                          <Box>
                            {" "}
          ...
          {tab === 2 && (
            <Box>
              ...
              {errorIngredients && (
                <Alert severity="error">{errorIngredients}</Alert>
              )}
            ...
            {({
              ...
              <Form>
                <DialogContent>
                  ...
                  >
                    <MenuItem value={ProductType.SINGLE_PRICE}>
                      Unique Price
                    </MenuItem>
                    <MenuItem value={ProductType.MULTIPLE_SIZES}>
                      Multiple Sizes
                    </MenuItem>
                  </TextField>
                        ...
                        <Box key={idx} sx={{ display: "flex", gap: 1, mb: 1 }}>
                          <TextField
                            ...
                            error={
                              ...
                              touched.sizes[idx]?.name &&
                              Array.isArray(errors.sizes) &&
                              Boolean((errors.sizes[idx] as any)?.name)
                            }
                            helperText={
                              Array.isArray(touched.sizes) &&
                              touched.sizes[idx]?.name &&
                              Array.isArray(errors.sizes) &&
                              (errors.sizes[idx] as any)?.name
                            }
                            sx={{ flex: 2 }}
                          />
                          <TextField
                            ...
                            error={
                              Array.isArray(errors.sizes) &&
                              Array.isArray(touched.sizes) &&
                              touched.sizes[idx]?.price &&
                              Boolean((errors.sizes[idx] as any)?.price)
                            }
                            helperText={
                              Array.isArray(errors.sizes) &&
                              Array.isArray(touched.sizes) &&
                              touched.sizes[idx]?.price &&
                              (errors.sizes[idx] as any)?.price
                            }
                            sx={{ flex: 1 }}
                          />
                          <IconButton
                  ...
                  <TextField
                    ...
                    value={
                      Array.isArray(values.ingredients)
                        ? values.ingredients.map((ing) => ing._id || ing)
                        : []
                    }
                    onChange={(e) => {
                  ...
                  <Box sx={{ mt: 2, mb: 1 }}>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              ...
                              if (values.file) {
                                URL.revokeObjectURL(
                                  URL.createObjectURL(values.file as File)
                                );
                              }
                    ...
                    {!values.file && values.image_url && (
                      <Box sx={{ mt: 1 }}>
                        ...
                        <Box sx={{ mt: 1 }}>
                          <img
                            src={`${process.env.NEXT_PUBLIC_API_URL}/${values.image_url}`}
                            alt="Current"
                            style={{
                              maxWidth: 120,
                              maxHeight: 120,
        ...
        >
          <DialogTitle>
            {ingredientEdit ? "Edit Ingredient" : "Add Ingredient"}
          </DialogTitle>{" "}
          <Formik
            initialValues={{
